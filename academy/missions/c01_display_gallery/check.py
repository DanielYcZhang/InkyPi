...
def check_mission():
    print("Initiating Display Gallery Verification...\n")
    all_passed = True

    repo_root = find_repo_root(Path.cwd())
    if not repo_root:
        print_result("Could not locate repo root (expected to find src/plugins).", False)
        print("  -> Hint: run this inside the InkyPi repo, e.g. ~/InkyPi/academy/missions/c01_display_gallery")
        return
    print_result(f"Repo root detected: {repo_root}", True)

    plugin_id = "c01_display_gallery"
    ...
    if _check_text_file(explain_path, 200, keywords=["image.new", "loop"]):
    ...
    if _check_text_file(mission_dir / "trace.md", 80):
    ...
    if _check_text_file(bad_code_py, 120, keywords=["def draw_boxes"]):
    ...
    print("\n-----------------------------")
    if all_passed:
        print(f"{GREEN}MISSION COMPLETE. The gallery is up.{RESET}")
    else:
