...
def check_mission():
    print("Initiating Inheritance Practice Verification...\n")
    all_passed = True

    repo_root = find_repo_root(Path.cwd())
    if not repo_root:
        print_result("Could not locate repo root (expected to find src/plugins).", False)
        print("  -> Hint: run this inside the InkyPi repo, e.g. ~/InkyPi/academy/missions/c03_inheritance_practice")
        return
    ...
    if _check_text_file(explain_path, 200, keywords=["super", "base", "class"]):
    ...
    if _check_text_file(bad_code_py, 120, keywords=["class PetCard"]):
    ...
    print("\n-----------------------------")
    if all_passed:
        print(f"{GREEN}MISSION COMPLETE. The inheritance works.{RESET}")
    else:
